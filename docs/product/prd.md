...
| --- | --- | --- |
| 會議名稱 | 是 | 預設為「會議通知」，可修改 |
| 會議時間 | 是 | 會議開始日期時間，作為 Google Calendar 事件開始時間 |
| 結束時間 | 否 | 未填時，Google Calendar 預設為開始時間後 1 小時 |
| 地點 | 否 | 顯示於通報內容 |
| 參加人員 | 否 | 顯示於通報內容 |
| 相關資料 | 否 | 顯示於通報內容 |
...
- 一般會議不使用 `actual_due_at` 或 `announced_due_at`，建立後預設為 `Completed`。
- 加入 Google Calendar 時，使用會議時間與結束時間建立定時事件；若未填結束時間，結束時間為開始後 1 小時。
