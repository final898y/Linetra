...
| 重要旗標 | Importance Flag | 標記案件為重要等級，觸發 LINE 通報中「@All 【重要】」前綴 |
...
So that LINE 通報中會自動加上「@All 【重要】」前綴，提高受通知者的注意力。
...
**規則**：
- `@All 【重要】` 只在 `importance_flag=true` 時顯示
...
**規則**：
- 格式為 `【 公 告 通 知 】`（無 `@All 【重要】` 前綴，即使重要旗標為 true）
- 不顯示分隔線 `~~~~~~~~~~~~~~~~~~~~~~~~~~`
- 內容段落為自由文字，使用者自行撰寫
...
**Then**
- 在 500ms 內顯示預覽區
- 預覽文字包含 `@All 【重要】`、`【 案 件 通 報 】`、`通報單位：人事處`、`案由：薪資調查表`、`期限：明天（週四）17:00 下班前`（假設今天週三）、`繳交方式：`、`- 紙本核章`
- 分隔線正確顯示（`~~~~~~~~~~~~~~~~~~~~~~~~~~`）
- 「複製」按鈕可點擊
